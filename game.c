...
void main() {
    ...
    NR51_REG = 0xFF;  // is 1111 1111 in binary, select which channels we want
                      //     to use in this case all of them. One bit for the
                      //     L one bit for the R of all four channels
    ...
    while (1) {
        ...
        if (joypad_state) {
            ...
            // 0001 0110 is 0x16, sweep time 1, sweep direction increase,
            //     shift amount per step 110 (6 decimal)
            NR10_REG = 0x16;
            ...
            // channel 1 register 2: Volume Envelope (Makes the volume get
            //      louder or quieter each "tick")
            ...
            //      register, sound can be produced consecutively.  When "1",
            //      sound is generated during the time period set by the length
            //      data contained in register NR11. After the sound is output,
            //      the Sound 1 ON flag, at bit 0 of register NR52 is reset.
            // 5-3 Unused
